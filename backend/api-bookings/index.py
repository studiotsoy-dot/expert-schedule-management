...
def handler(event: dict, context) -> dict:
    ...
    if method == 'POST' and path.rstrip('/').endswith('/bookings'):
        ...
        cur.execute(
            ...
            (booking_id, slot_id, body['manager_id'], body['client_name'],
             body.get('client_phone', ''), body.get('client_email', ''), zoom_link, now)
        )
    ...
    if method == 'POST' and path.endswith('/reschedule'):
        ...
        cur.execute(
            ...
            (new_slot_id, old_slot[0], body['new_date'], body['new_start_time'], body['new_end_time'])
        )
        ...
        conn.commit()
        conn.close()
        return resp(200, {'new_slot_id': new_slot_id, 'date': body['new_date'], 'start_time': body['new_start_time']})
